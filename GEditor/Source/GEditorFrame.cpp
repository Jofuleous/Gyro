...
#include "TypeFrames/GEWaypointFrame.h"
...
BEGIN_EVENT_TABLE(GEditorFrame,wxFrame)
	...
	EVT_MENU( ID_CREATE_WAYPOINT, GEditorFrame::CreateWaypoint )
...
void GEditorFrame::CreateGUIControls()
{
	...
	ID_CREATE_Mnu_Obj->Append(ID_CREATE_WAYPOINT, _("Waypoint"), _(""), wxITEM_NORMAL);
...
void GEditorFrame::CreateWaypoint( wxCommandEvent& event )
{
	g_EditorScene::Get().CreateWaypoint();
}
...
void GEditorFrame::Property_Translate_XUpdated(wxCommandEvent& event)
{
	...
	float xVal = (float) what;

	GEditorObject* obj = g_EditorScene::Get().m_selectedObject;
	if( obj )
	{
		GVector3 x( xVal, obj->m_position._y, obj->m_position._z );
		obj->UpdatePosition( x );
	}
	else if( g_EditorScene::Get().m_path.m_selectedNode != MAX_U16 )
	{
		// hack.
		GVector3 x = g_EditorScene::Get().m_path.GetNodePosition( g_EditorScene::Get().m_path.m_selectedNode );
		x.x( xVal );
		g_EditorScene::Get().m_path.SetNodePosition( g_EditorScene::Get().m_path.m_selectedNode, x );
	}
...
void GEditorFrame::Property_Translate_YUpdated(wxCommandEvent& event)
{
	...
	float yVal = (float) what;

	if( obj )
	{
		GVector3 y( obj->m_position._x, yVal, obj->m_position._z );
		obj->UpdatePosition( y );
	}
	else if( g_EditorScene::Get().m_path.m_selectedNode != MAX_U16 )
	{
		GVector3 y = g_EditorScene::Get().m_path.GetNodePosition( g_EditorScene::Get().m_path.m_selectedNode );
		y.y( yVal );
		g_EditorScene::Get().m_path.SetNodePosition( g_EditorScene::Get().m_path.m_selectedNode, y );
	}
...
void GEditorFrame::Property_Translate_ZUpdated(wxCommandEvent& event)
{
	...
	if( obj )
	{
		if( event.GetEventType() == wxEVT_TEXT_ENTER )
		{
			GVector3 z( obj->m_position._x, obj->m_position._y, zVal );
			obj->UpdatePosition( z );
		}
	}
	else if( g_EditorScene::Get().m_path.m_selectedNode != MAX_U16 )
	{
		GVector3 z = g_EditorScene::Get().m_path.GetNodePosition( g_EditorScene::Get().m_path.m_selectedNode );
		z.z( zVal );
		g_EditorScene::Get().m_path.SetNodePosition( g_EditorScene::Get().m_path.m_selectedNode, z );
	}
...
{
	if( g_EditorScene::Get().m_path.m_selectedNode != MAX_U16 )
		DisplayTypeFrame( NULL );
	else if( !m_typeFrame || (g_EditorScene::Get().m_selectedObject && g_EditorScene::Get().m_selectedObjects.size() == 1) )
	{
...
void GEditorFrame::DisplayWayNode( u16 i_node )
{
	GVector3 pos = g_EditorScene::Get().m_path.GetNodePosition( i_node );
	int nodeInt = i_node;
	char buffer[64];
	// Translation
	sprintf( buffer, "%f", pos._x );
	Property_Translate_X->ChangeValue( buffer );
	sprintf( buffer, "%f", pos._y );
	Property_Translate_Y->ChangeValue( buffer );
	sprintf( buffer, "%f", pos._z );
	Property_Translate_Z->ChangeValue( buffer );

	// Scale
	Property_Scale_X->ChangeValue( "1.0" );
	Property_Scale_Y->ChangeValue( "1.0" );
	Property_Scale_Z->ChangeValue( "1.0" );

	// Rotate
	Property_Rotate_X->ChangeValue( "0.0" );
	Property_Rotate_Y->ChangeValue( "0.0" );
	Property_Rotate_Z->ChangeValue( "0.0" );

	sprintf( buffer, "%d", nodeInt );
	Property_Object_Name->ChangeValue( buffer );
	Property_Type->SetSelection( GEditorObject::WAYPOINTSYS );
...
void GEditorFrame::DisplayTypeFrame( GEditorObject* i_obj )
{
	...
	if( i_obj )
	{
		switch( i_obj->m_type )
		{
			...
			break;
			default:
				newTypeFrame = new GEWaypointFrame( this ); // hack
			break;
		}
	}
	else
		newTypeFrame = new GEWaypointFrame( this );
	...
	if( newTypeFrame )
	{
		...
		if( i_obj )
			...
			m_typeFrame->DisplayWayNode( g_EditorScene::Get().m_path.m_selectedNode ); // hack
